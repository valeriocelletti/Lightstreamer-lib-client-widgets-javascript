  ...
  function updateContent(val,useInner) {
    if (useInner) {
      ...
      if (this.el.childNodes.length != 1 || this.el.firstChild.nodeType != 3) {
        // passiamo di qua se
        // *nella cella c'è dell'html (più di un figlio indica almeno un tag; un figlio ma non
        //  nodeType 3 non è un nodo di testo): dagli una pulita
        // *oppure la cella è completamente vuota (no firstChild):
        //  e' stata creata tramite LS_cell e il suo contenuto era vuoto
        if (this.el.firstChild != null) {
          this.el.innerHTML = "";
        }
        this.el.appendChild(document.createTextNode(val));
      } else {
        // se la cella e' stata presa in carico da convertHtmlStructures
        // e' sicuramente un TextNode; 
        // se la cella è già stata aggiornata una volta in qursta vita
        // è sicuramente un TextNode
        // in ogni caso il test fa si che qui ci sia per forza un TextNode
