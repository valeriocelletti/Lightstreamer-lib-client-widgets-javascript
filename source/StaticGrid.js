...
define(["Inheritance","./AbstractGrid","./VisualUpdate","./Cell","./SlidingCell","./CellMatrix",
   ...
   * 
   * @param {DOMElement} rootEl if specified, the cells to make up the HTML grid will
   * only be searched in the list of descendants of this node. Equivalent to a
   * {@link #setRootNode} call, but useful if autoParse is true.
   * 
   * @param {Array} cells an array of DOMElement instances that will make up the
   ...
   * the single cells during {@link #onVisualUpdate} executions.</li>
   * </ul>
   * <BR>
   ...
   * specified in the {@link #setRootNode} are taken into account, unless a list
   * of cells has been manually specified in the constructor or through the {@link #addCell}
...
//////////////////////////////////TEMPLATE SETUP       
       ...
       *
       * @param {DOMElement} cellElement A DOM pointer to an HTML node.
       ...
       * the {@link #addCell} method, then the search will not be performed at all.
       *
       ...
       * a call to {@link AbstractGrid#parseHtml} is necessary.</p>
       *
       * @param {DOMElement} rootNode a DOM node to be used as starting point
       ...
       * 
       * @return {Array} The list of unique values found in the "data-item" properties
...
////////INITIAL SORT
      ...
      sortTable: function() {
        ...
        for (var i in newRowToKey) {
          ...
          oldKeyPerPos = this.keyRowMap.getReverse(i);
       ...
       * instances (or even N times to the same one, in which case it will receive
       * duplicated events and has to be removed N times before stopping receiving
       * them).
       * 
       ...
       * 
       * @return {Array} an array containing the listeners that were added to this instance.
